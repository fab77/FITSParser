 ...
 */
var ParseUtils = /** @class */ (function () {
    function ParseUtils() {
    }
    ParseUtils.getStringAt = function (data, offset, length) {
        var chars = [];
    ...
    ParseUtils.byteString = function (n) {
        if (n < 0 || n > 255 || n % 1 !== 0) {
            throw new Error(n + " does not fit in a byte");
        }
        return ("000000000" + n.toString(2)).substr(-8);
    };
    ParseUtils.parse32bitSinglePrecisionFloatingPoint = function (byte1, byte2, byte3, byte4) {
        var long = (((((byte1 << 8) + byte2) << 8) + byte3) << 8) + byte4;
        if (long < 0)
            long += 4294967296;
        var float = (1.0 + (long & 0x007fffff) / 0x0800000) *
            Math.pow(2, ((long & 0x7f800000) >> 23) - 127);
        return float;
    };
    ParseUtils.convertBlankToBytes = function (blank, nbytes) {
        var str = Math.abs(blank).toString(2);
    ...
    ParseUtils.parseFloatingPointFormat = function (bytes, ebits, fbits) {
        // Bytes to bits
        var bits = [];
    ...
    ParseUtils.generate16bit2sComplement = function (val) {
        throw new TypeError("not implemented yet" + val);
    };
    ParseUtils.parse16bit2sComplement = function (byte1, byte2) {
        var unsigned = (byte1 << 8) | byte2;
        if (unsigned & 0x8000) {
    ...
    ParseUtils.parse32bit2sComplement = function (byte1, byte2, byte3, byte4) {
        var unsigned = (byte1 << 24) | (byte2 << 16) | (byte3 << 8) | byte4;
        var s = (unsigned & 0x80000000) >> 31;
        var res = unsigned & 0xffffffff;
        if (s) {
     ...
     */
    ParseUtils.getByteAt = function (data, offset) {
        var dataOffset = 0;
        return data.charCodeAt(offset + dataOffset) & 0xff;
    };
    ParseUtils.extractPixelValue = function (offset, bytes, bitpix) {
        var px_val = undefined; // pixel value
