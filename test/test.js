...
describe("Reading FITS file from URL:", function() {
    it("Reading HiPS FITS", (done) =>  {
        let fileuri = "http://skies.esac.esa.int/Herschel/normalized/PACS_hips160//Norder8/Dir40000/Npix47180.fits";
        
        new FITSParser(fileuri).then(fits => {
            console.log(fits.header);
            FITSParser.writeFITS(fits.header, fits.data)
            done();
        })
        .catch(done);
    });
...
// describe("Reading FITS file:", function() {
//     it("type:HiPS, from Herschel using Promise", (done) =>  {
//         let fileuri = "/Users/fgiordano/Workspace/GitHub/FITSParser/test/inputs/x0c70103t_c1f.fits";
//         new FITSParser(fileuri).then(fits => {
//             console.log(fits.header);
//             // assert.equal(fits.header.get('NPIX'), '278');
//             FITSParser.writeFITS(fits.header, fits.data)
